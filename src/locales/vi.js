const translationVI = {
  header: {
    'option_en.lbl': 'English',
    'option_ko.lbl': '한국어',
    'option_vi.lbl': 'Tiếng Việt',
    'download.btn': 'TẢI VỀ',
    'introduction.btn': 'Giới thiệu',
    'key-features.btn': 'Tính năng',
    'core-skills.btn': 'Kỹ năng',
    'our-methods.btn': 'Phương pháp',
    'reviews.btn': 'Đánh giá',
    'explore.btn': 'Khám phá',
    'roadmap.btn': 'Lộ trình',
    'teams.btn': 'Đội ngũ phát triển',
    'contact.btn': 'Liên hệ',
  },

  modalDownload: {
    titleMain: 'Chọn thiết bị tải về',
  },

  overview: {
    'title.lbl': 'METALISH',
    'content_1.txt':
      'Metalish là một ứng dụng học tiếng Anh dựa trên chương đào tạo của bộ Giáo dục & Đào tạo, tập trung vào các kỹ năng tiếng Anh chính: nghe, nói, từ vựng, khẩu hình miệng. Mục đích của chúng tôi là phát triển một môi trường cho người dùng để có thể sử dụng tiếng Anh lưu loát trong cuộc sống hàng ngày.',
    'content_2.txt':
      'Metalish mang đến cho người dùng những trải nghiệm thực tế khi học tiếng Anh, cung cấp những phương pháp học đa dạng, lộ trình học tập riêng. Những bài học đều sẽ bao gồm các minigame hấp dẫn.',
    'content_3.txt':
      'Bên cạnh đó, Metalish luôn đón nhận mọi đánh giá của người dùng để cải thiện chất lượng dạy học. Những đánh giá hiệu quả cũng giúp người dùng phản ánh được quá trình học tập của họ, từ đó họ có thể điều chỉnh tốt hơn quá trình học tập của mình.',
    'content_4.txt':
      'Hơn thế nữa, Metalish hy vọng có thể tạo ra được một không gian quen thuộc cho tất người dùng. Người dùng có thể thiết kế lớp học của họ và mời bạn bè cùng tham gia. Chúng tôi muốn mang đến trải nghiệm người dùng tốt nhất.',
    'content_5.txt':
      'Hãy cùng bắt đầu khám phá thế giới tiếng Anh của riêng bạn nào!',
    'model.lbl': 'Kéo sang trái hoặc phải để xem không gian 3D',
  },
  'key-features': {
    'title.lbl': 'CÁC TÍNH NĂNG CHÍNH',
    'studying-english_title.lbl': 'Học tiếng Anh',
    'studying-english_content.txt':
      'Metalish là một ứng dụng giúp người dùng cải thiện khả năng tiếng Anh của bản thân. Được phát triển dựa theo chương trình đào tạo của Bộ Giáo dục.',

    'graphics_title.lbl': 'Đồ họa',
    'graphics_content.txt':
      'Tạo ấn tượng với người dùng bằng đồ họa 3D, giao diện thân thiện người dùng, dễ sử dụng.',

    'connect_title.lbl': 'Kết nối',
    'connect_content.txt':
      'Kết bạn và kết nối với giáo viên để hiểu rõ bài học.',

    'customized-classroom_title.lbl': 'Lớp học tùy chỉnh',
    'customized-classroom_content.txt':
      'Người dùng có thể tự do điều chỉnh giao diện lớp học theo sở thích cá nhân. ',

    'practical-english_title.lbl': 'Tiếng Anh thực hành',
    'practical-english_content.txt':
      'Giúp người dùng luyện tập tiếng Anh bằng việc cung cấp các đoạn hội thoại thực tế.',

    'ai-technology_title.lbl': 'Công nghệ AI',
    'ai-technology_content.txt':
      'Dùng công nghệ AI để tương tác với người dùng và chỉnh phát âm.',

    'ai-in-advertising_title.lbl': 'AI trong quảng cáo',
    'ai-in-advertising_content.txt':
      'Cung cấp quảng cáo cá nhân và hỗ trợ mua bán thông qua quảng cáo.',

    'mini-games_title.lbl': 'Mini games',
    'mini-games_content.txt':
      'Thực hành tiếng Anh bằng việc chơi các trò chơi mini: kiểm tra từ vựng, phát âm,...',

    'rewards_title.lbl': 'Phần thưởng',
    'rewards_content.txt':
      'Nhận được phiếu mua hàng, giảm giá khi mua những khóa học mới hay phiếu mua sách/đồ vật được quảng cáo trong app.',

    'best-choice_title.lbl': 'Lựa chọn tốt nhất cho học sinh',
    'best-choice_content.txt':
      'Tập trung vào đối tượng học sinh từ lớp 1 đến lớp 12. Thích hợp cho các quốc gia mà tiếng Anh không phải tiếng mẹ đẻ, đặc biệt là Đông Nam Á.',

    'review_title.lbl': 'Đánh giá ',
    'review_content.txt':
      'Người dùng sẽ được nhận phần thưởng cho mỗi đánh giá hiệu quả về app, khóa học, đội ngũ giảng viên của chúng tôi.',
  },

  'core-skills': {
    'title.lbl': 'CÁC KỸ NĂNG CHÍNH',
    'listening.lbl': 'Nghe',
    'speaking.lbl': 'Nói',
    'vocabulary.lbl': 'Từ vựng',
    'mouth-shape.lbl': 'Khẩu hình miệng',
  },

  'our-methods': {
    'title.lbl': 'PHƯƠNG PHÁP CỦA CHÚNG TÔI',
    'private-room_title.lbl': 'PHÒNG CÁ NHÂN',
    'talking-room_title.lbl': 'PHÒNG LUYỆN NÓI',
    'private-room_content.txt':
      'Cung cấp lớp tự học với các video được quay từ những giáo viên tiếng Anh chất lượng.\n' +
      'Cải thiện các kỹ năng tiếng Anh: nghe, nói, đọc, viết, khẩu hình miệng.',
    'talking-room_content.txt':
      'Trả thêm một khoản phí nhỏ để tham gia các lớp học cá nhân được tùy chỉnh với AI hoặc với giáo viên mà bạn thích. \n' +
      'Những chủ đề không giới hạn.\n' +
      'Mời bạn bè cùng đi vào thế giới của mình.',
    'cooperation_content.txt':
      'Đội ngũ cố vấn nội dung giàu kinh nghiệm, bao gồm giáo viên tiếng Anh nước ngoài và giáo viên tiếng Anh bản địa.',
    'curriculum_content.txt':
      'Chương trình đào tạo hoàn toàn dựa trên chương trình đào tạo tại quốc gia của người dùng.',
  },

  feedback: {
    'title.lbl': 'NGƯỜI DÙNG NÓI GÌ VỀ METALISH',
    'user-1-name.txt': 'Min Nguyen',
    'user-1-description.txt': 'Học sinh lớp 5',
    'user-1-summary-comment.lbl': 'Tiện lợi',
    'user-1-full-comment.txt':
      'Mỗi khóa học đều rất vui và tạo động lực học. Tôi có thể học bất cứ khi nào tùy thuộc vào lịch trình và tâm trạng của mình.',

    'user-2-name.txt': 'Jin Nguyen',
    'user-2-description.txt': 'Học sinh lớp 12',
    'user-2-summary-comment.lbl': 'Trải nghiệm thú vị và tuyệt vời',
    'user-2-full-comment.txt':
      'Tôi đã có được sự tự tin khi dùng tiếng Anh trong cuộc sống hàng ngày. Với Metalish, tôi có thể gặp gỡ và tương tác với những người có cùng sở thích với mình.',

    'user-3-name.txt': 'Joy Le',
    'user-3-description.txt': 'Học sinh lớp 12',
    'user-3-summary-comment.lbl': 'Đội ngũ giảng viên chuyên nghiệp',
    'user-3-full-comment.txt':
      'Phản hồi của giảng viên luôn luôn kịp thời và mang tính xây dựng.',

    'user-4-name.txt': 'Samuel Nguyen',
    'user-4-description.txt': 'Học sinh lớp 5',
    'user-4-summary-comment.lbl': 'Userflow tuyệt vời',
    'user-4-full-comment.txt':
      'App này dễ dùng, tôi biết mình cần click vào đâu và không click vào đâu.',

    'user-5-name.txt': 'Rift Ho',
    'user-5-description.txt': 'Học sinh lớp 12',
    'user-5-summary-comment.lbl': 'Thiết kế tuyệt đẹp',
    'user-5-full-comment.txt':
      'Giao diện nhiều màu sắc và tươi sáng, giúp tôi có thể tập trung lâu hơn vào việc học của mình.',

    'user-6-name.txt': 'Leon Nguyen',
    'user-6-description.txt': 'Học sinh lớp 5',
    'user-6-summary-comment.lbl': 'Trải nghiệm đỉnh nhé',
    'user-6-full-comment.txt':
      'Tôi chỉ cần trả thêm một khoản phí nhỏ đã có thể tham gia những lớp học cá nhân tùy chỉnh với AI hoặc giáo viên mà mình thích.',

    'user-7-name.txt': 'Funik Nguyen',
    'user-7-description.txt': 'Học sinh lớp 12',
    'user-7-summary-comment.lbl': 'Giáo trình được chuẩn bị kỹ lưỡng',
    'user-7-full-comment.txt':
      'Những bài học được chuẩn bị kỹ lưỡng và chi tiết. Luôn có câu trả lời cho mọi câu hỏi của tôi.',

    'user-8-name.txt': 'Talon Nghiem',
    'user-8-description.txt': 'Học sinh lớp 5',
    'user-8-summary-comment.lbl': 'Lớp học rất tuyệt vời',
    'user-8-full-comment.txt':
      'Có 2 kiểu lớp học. Và lớp cá nhân là lớp yêu thích của tôi, lớp này giúp tôi tự học với các video được quay lại bởi những giảng viên tiếng Anh.',
  },

  explore: {
    titleMain: 'Cùng chúng tôi khám phá Metalish nào!',
  },
  tour: {
    titleMain: 'KHÁM PHÁ METALISH',
  },
  miniGames: {
    titleMain: 'MINI GAMES',
    blank: 'Điền vào chỗ trống',
    choice: 'Trắc nghiệm',
    word: 'Sắp xếp từ',
  },
  advertising: {
    titleMain: 'BẢNG QUẢNG CÁO 3D',
    title1: `“Mẫu vali này đang được bán tại Gmarket.co.kr...”`,
    title2: `“Tôi có thể mua vali này ở đâu?”`,
  },
  orderService: {
    titleMain: 'DỊCH VỤ ĐẶT HÀNG',
    titleDes: 'Tương tác với quảng cáo để đặt hàng thực tế',
  },
  roadmap: {
    titleMain: 'LỘ TRÌNH',
    title2022: {
      month: 'AUG',
      title1: 'Ý tưởng cho Metalish',
      title2: 'Sắp xếp nguồn ý tưởng và khởi động',
      title3: 'Phát triển Metalish phase 1',
      title4: 'Thiết kế art cho phase 1',
      title5: 'Ra mắt website',
      title6: 'Chuẩn bị cho phase 2',
    },
    title2023: {
      month: 'FEB',
      title1: 'Ra mắt version 1',
      title2: 'Thiết kế art cho phase 2',
      title3: 'Phát triển Metalish phase 2 (metaverse)',
      title4: 'Nâng cấp server (hơn 10 triệu người dùng)',
      title5: 'Kêu gọi vốn đầu tư',
      title6: 'Chạy quảng cáo trong ứng dụng',
      title7: 'Chuẩn bị cho phase 3',
    },
    title2023Second: {
      month: 'AUG',
      title1: 'Ra mắt version 2',
      title2: 'Thiết kế art cho phase 3',
      title3: 'Phát triển Metalish phase 3',
      title4: 'Nâng cấp AI và UI',
    },
    title2024: {
      month: 'FEB',
      title1: 'Ra mắt version 3',
      title2: 'Ra mắt app (metaverse)',
    },
  },
  developmentTeam: {
    titleMain: 'ĐỘI NGŨ PHÁT TRIỂN',
  },
  footer: {
    titleMain: 'Thông tin liên lạc',
    address: 'Số 82 đường số 6, phường 10, quận Gò Vấp, Tp. Hồ Chí Minh',
    email: 'metalish@cuckookoo.vn',
    phone: '0336111248',
  },
};

export default translationVI;
